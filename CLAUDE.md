...
- `pnpm test` — run all tests (Vitest; Node environment by default, component tests opt into jsdom via a per-file `// @vitest-environment jsdom` pragma)
...
**FSRS state** lives as columns on `cards` (mirroring the ts-fsrs `Card` type) and `review_logs` (mirroring `ReviewLog`). `State.New` (0) marks unstudied cards; the study queue orders due cards oldest-day-first (shuffled within each day) and evenly interleaves a capped number of new cards (selected oldest-first, presented shuffled) — randomized to avoid sequence-memorization effects, on top of FSRS interval fuzz (`enable_fuzz` in `src/srs/scheduler.ts`).
